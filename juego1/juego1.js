...
lib.ssMetadata = [
		{name:"juego1_atlas_1", frames: [[0,0,1650,1275]]}
];
...
(lib.img1 = function() {
	this.initialize(ss["juego1_atlas_1"]);
	this.gotoAndStop(0);
...
if (reversed == null) { reversed = false; }
	...
	// Capa_1
	this.shape = new cjs.Shape();
	var sprImg_shape = cjs.SpriteSheetUtils.extractFrame(ss["juego1_atlas_1"],0);
...
if (reversed == null) { reversed = false; }
	...
	this.frame_0 = function() {
		...
		// ==============================
		var txtFelicitaciones = new createjs.Text(
		    "¡FELICITACIONES!",
		    "bold 36px Arial",
		    "#28a745"
		);
		txtFelicitaciones.x = 140;
		txtFelicitaciones.y = 30;
		txtFelicitaciones.visible = false;
		stage.addChild(txtFelicitaciones);
		...
		function soltar(evt) {
		    ...
		    if (zonas[i].hitTest(pt.x, pt.y) && !pieza.colocada) {
		
		        pieza.colocada = true;
		
		        // 🔧 AJUSTE FINAL SOLO PARA PIEZA 1
		        if (i === 0) {
		            pieza.x = finalPos[i].x + 4;
		        ...
		        pieza.mouseEnabled = false;
		
		        piezasCorrectas++;
		
		        // 🎉 CUANDO SE COMPLETA EL ROMPECABEZAS
		        if (piezasCorrectas === piezas.length && !mensajeMostrado) {
		            mensajeMostrado = true;
		            txtFelicitaciones.visible = true;
		        }
	...
	// piezas
	this.txtp = new cjs.Text("puntaje", "bold 30px 'Times New Roman'", "#FFFFFF");
	this.txtp.name = "txtp";
	this.txtp.textAlign = "center";
	this.txtp.lineHeight = 35;
	this.txtp.lineWidth = 100;
	this.txtp.parent = this;
	this.txtp.setTransform(91.05,346.35);
	...
	this.timeline.addTween(cjs.Tween.get({}).to({state:[{t:this.ipieza1},{t:this.ipieza2},{t:this.ipieza3},{t:this.ipieza4},{t:this.ipieza5},{t:this.ipieza6},{t:this.txtp}]}).wait(1));
...
lib.properties = {
	...
	opacity: 1.00,
	manifest: [
		{src:"images/juego1_atlas_1.png?1768432059293", id:"juego1_atlas_1"}
	],
	preloads: []
};
